...
pub fn build(b: *std.Build) void {
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            .target = b.resolveTargetQuery(.{ .cpu_arch = .arm, .os_tag = .linux, .abi = .gnueabi, .glibc_version = .{ .major = 2, .minor = 23, .patch = 0 } }),
        }),
    });

    exe.addIncludePath(b.path("sdk/include"));
    exe.addLibraryPath(b.path("sdk/lib"));
...
const SendTar = struct {
    ...
    fn make(step: *Step, options: Step.MakeOptions) !void {
        ...
        var install_walker = try install_dir.walk(std.heap.page_allocator);
